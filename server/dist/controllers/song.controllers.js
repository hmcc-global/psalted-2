...
const getSong = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (songId) {
        try {
            const data = yield song_model_1.Song.findOne({ _id: songId, isDeleted: false }).exec();
            if (data) {
                sendResponse(res, 200, data);
            }
            else {
    ...
    if (Object.keys(filter).length > 0) {
        ...
        try {
            const data = yield song_model_1.Song.find({ isDeleted: false })
                .select('_id title tempo originalKey themes artist year code lyricsPreview isVerified isDeleted createdAt updatedAt')
                .exec();
            if (data) {
                sendResponse(res, 200, data);
            }
